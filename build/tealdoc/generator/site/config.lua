...
function SiteConfig.configure(
   ...
   normalize_sidebar_item = function(
      ...
      assert(
      item["path"] ~= nil or
      type(item["text"]) == "string" and item["text"] ~= "",
      ...
      local normalized = {
         text = (item["text"] or
         path and pages_by_path[path].title),
         path = path,
         items = {},
         collapsed = item["collapsed"] == true,
      }
      for child_index, child in ipairs(item["items"] or {}) do
         table.insert(
         normalized.items,
   ...
   for index, item in ipairs(values["sidebar"] or {}) do
      table.insert(
      settings.sidebar,
      normalize_sidebar_item(
      item,
