...
function CommentParser.parse_lines(lines, item, env)
   ...
   local function end_tag()
      ...
      env.tag_handler_registry[current_tag].handle(current_context)
      current_context = nil
      current_tag = nil
   end

   local function start_tag(handler, param, description, item)
      in_text = false
      end_tag()
   ...
   for _, line in ipairs(lines) do
      ...
      for tag_name, handler in pairs(env.tag_handler_registry) do
         ...
         elseif handler.has_description then
            ...
            if description then
               has_tag = true
               start_tag(handler, nil, description, item)
            end
         else
            local pattern = "@" .. tag_name
            if trimmed:match(pattern) then
               has_tag = true
               start_tag(handler, nil, nil, item)
            end
         end
      end
